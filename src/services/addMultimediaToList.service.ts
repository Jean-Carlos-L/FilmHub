...
export const addMultimediaToListService = (fetch: FetchCustom) => async (idMultimedia, idList) => {
   try {
      ...
      const list = getLIsts();
      const multimedia = initialState.find((multimedia) => multimedia.id === idMultimedia);

      const listsId = list.findIndex((list) => list.id === idList);

      list[listsId].multimedia.push(multimedia);

      localStorage.setItem('lists', JSON.stringify(list));

      return 'Multimedia agregada a la lista correctamente'
   } catch (error) {
